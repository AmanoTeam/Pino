...
if ! [ -f "${gcc_tarball}" ]; then
	...
	patch --directory="${gcc_directory}" --strip='1' --input="${workdir}/submodules/obggcc/patches/0001-Link-libgcobol-with-libiconv.patch"
...
for triplet in "${targets[@]}"; do
	...
	echo 'GROUP ( ../libc.so AS_NEEDED ( ../libc.a ../libiconv.so ../libcharset.so ) )' > "${sysroot_directory}/lib/ldscripts/libc.so"
	...
	../configure \
		...
		--with-pkgversion="Pino v1.2-${revision}" \
		--with-sysroot="${toolchain_directory}/${triplet}" \
