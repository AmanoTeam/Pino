...
for triplet in "${targets[@]}"; do
	...
	env ${args} make \
		${subargs} \
		CFLAGS_FOR_TARGET="-save-temps -fuse-ld=${linker} -D __BUILDING_GCC_TARGET_LIBRARIES__ -D __ANDROID_MIN_SDK_VERSION__=${base_version} -D __ANDROID_API__=${base_version} ${optflags} ${linkflags}" \
		CXXFLAGS_FOR_TARGET="-save-temps -fuse-ld=${linker} -D __BUILDING_GCC_TARGET_LIBRARIES__ -D __ANDROID_MIN_SDK_VERSION__=${base_version} -D __ANDROID_API__=${base_version} ${optflags} ${linkflags}" \
